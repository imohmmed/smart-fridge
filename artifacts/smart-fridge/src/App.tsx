...
function SettingsPage({ user, data, setData, setNotice, onUserUpdate }: { user: User; data: UserData; setData: Dispatch<SetStateAction<UserData>>; setNotice: (v: string) => void; onUserUpdate: (updates: UserProfileUpdate) => void }) {
  ...
  return <main className="app-main">
    <PageHeading title={text(language, 'الإعدادات', 'Settings')} description={text(language, 'اجعل ثلاجتي تشبه طريقتك أكثر.', 'Make Smart Fridge feel more like you.')} hideMenu />
    <div className="settings-grid">
      ...
      <div className="card card-pad">
        ...
        {section === 'التنبيهات' && <><div className="card-title"><div><h3>{text(language, 'تنبيهات لطيفة', 'Gentle reminders')}</h3><p>{text(language, 'نذكّرك عندما يكون الوقت مناسباً', 'We remind you when the timing is right')}</p></div><Bell size={20} color="hsl(var(--primary))" /></div>
          <div className="setting-line"><div><strong>{text(language, 'تذكير انتهاء الصلاحية', 'Expiry reminder')}</strong><p>{text(language, 'قبل يومين من انتهاء الطعام', 'Two days before food expires')}</p></div><button className={`toggle ${data.reminders ? 'on' : ''}`} onClick={() => setData(prev => ({ ...prev, reminders: !prev.reminders }))} aria-label={text(language, 'تبديل تذكير انتهاء الصلاحية', 'Toggle expiry reminders')} aria-pressed={data.reminders} data-testid="toggle-reminders"><i /></button></div>
          <div className="setting-line"><div><strong>{text(language, 'ملخص نهاية اليوم', 'End-of-day summary')}</strong><p>{text(language, 'لمحة عن الماء والسعرات', 'A snapshot of water and calories')}</p></div><button className={`toggle ${data.notifications ? 'on' : ''}`} onClick={() => setData(prev => ({ ...prev, notifications: !prev.notifications }))} aria-label={text(language, 'تبديل ملخص نهاية اليوم', 'Toggle end-of-day summary')} aria-pressed={data.notifications} data-testid="toggle-notifications"><i /></button></div>
        </>}
